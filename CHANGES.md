...
- Validation: Build output diff shows only `export default function App()` → `function App()` change. `workspaceSymbol` confirmed no regression. Smoke test passed (preview.html renders correctly). Claude.ai sandbox no longer renders the artifact (needs `export default`), but sandbox compatibility is no longer required.
