...
- Notes:
	...
	- Routed Gmail labels are part of the intake contract for `adam@freedsolutions.com`: `Primitiv` for forwarded Outlook mail, `Primitiv/PRI_Teams` for Teams notifications, `LinkedIn` for LinkedIn message notifications, and `DMC/DMC_GMail` for DMC routed company mail.
