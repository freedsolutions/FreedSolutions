...
**Type = "Task"** — items Adam needs to do (personal tasks, decisions, deliverables)
**Type = "Follow Up"** — items involving another person (outreach, waiting on someone, delegated work, follow-up needed)
**Ambiguous items** — default to Task. Adam would rather review and reclassify than miss something.
...
| Description | Full context from the action item + which meeting it came from | Full context from the action item + meeting reference |
...
- **Properties:** Task name (title), Type (select: Task/Follow Up), Status (status), Priority (select: High/Low/Backburner), Record Status (select: Draft/Active/Inactive/Delete), Description (text), Due Date (date), Contact (relation → Contacts DB), Company (relation → Companies DB), Assignee (person), Summary (text), Source Meeting (relation → Meetings DB), Wiring Check (formula — flags Type/Assignee mismatches and missing Company)
...
**Page icon convention (manual, Session 19):** When Adam reviews Action Items, set the Notion page icon to match the Type. Agents cannot set page icons programmatically — this is a manual step during review.
- 📋 → Task
- ☝️ → Follow Up
