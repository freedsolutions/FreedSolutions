...
- **Claude project baseline** lives in `.claude/settings.json` and `.claude/settings.local.json`. Keep the shared Notion-workspace baseline entries aligned between both files: `mcp__notion`, `mcp__google-workspace`, `mcp__playwright`, the still-needed legacy `mcp__claude_ai_Notion__notion-{fetch,search,update-page,create-pages}` entries, `mcp__claude_ai_Gmail__gmail_read_message`, repo-scoped discovery shell commands, and the script-specific approvals for `compare-notion-sync.ps1`, `test-closeout-sanity.ps1`, `publish-codex-skills.ps1`, and `sync-claude-skill-wrappers.ps1`. Claude MCP permissions do not support `*` wildcards, so approve the server name itself when the intent is "all tools from this MCP server." Workstation extras outside that shared baseline may live in `.claude/settings.local.json`, but they must not replace missing baseline entries. `enableAllProjectMcpServers` should stay on, and `enabledMcpjsonServers` should include `playwright`.
