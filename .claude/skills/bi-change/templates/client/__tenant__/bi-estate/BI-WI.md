<!-- Generated from "freed-solutions/skills/bi-change/templates/client/__tenant__/bi-estate/BI-WI.md". Edit the repo skill source and rerun ops/notion-workspace/scripts/sync-claude-skill-wrappers.ps1; do not edit this Claude copy directly. -->

# <TENANT> BI — Work Instructions

Quick reference for using the BI dashboards: what each tile shows, and what its flags mean.
Everything else (process, change history, full detail) lives in `BI-SOP.md`.
**Last synced: <date>** — scaffolded from the `bi-change` client template.

The page shape below is BI-SOP Appendix A2 and is the ONLY authority on it; copy it once per
board and delete this note when the first board lands.
